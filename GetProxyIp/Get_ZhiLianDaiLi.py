...
class GetProxyIP_ZhiLianDaiLi:
    def __init__(self):
        self.url = 'http://http.zhiliandaili.cn/'

    # 从智联代理网站上获取随机的代理IP
    def get_ip_file(self, url):
        list = []
        headers = {'User-Agent': get_random_ua()}
        # 访问智联代理网站国内高匿代理，找到所有的tr节点对象
        ...
        for tr in tr_list[1:]:
            ip = MakeStandardIpAddress(tr.xpath('./th[1]/text()')[0])
            port = MakeStandardPort(tr.xpath('./th[2]/text()')[0])
            https = 'http'
